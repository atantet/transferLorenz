...
# propagate
p = [cont, cfg.model.sigma, cfg.model.beta]
xt = propagateRK4(po[t] + 5, field, p, dt, nt, samp=samp)
xt = xt[nt / samp / 10:-samp]
ax.plot(xt[:, 0], xt[:, 1], xt[:, 2], '-b')
plt.savefig('%s/continuation/chaoticOrbit_rho%04d%s.eps' \
            % (plotDir, cont*1000, dstPostfix), dpi=300, bbox_inches='tight')
