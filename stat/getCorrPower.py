...
ccf = ergoStat.ccf(observable1, observable2, lagMax=cfg.stat.lagMax,
                   sampFreq=sampFreq, norm=cfg.stat.norm)
...
# Save results
os.system('mkdir -p %s/correlation 2> /dev/null' % cfg.general.resDir)
os.system('mkdir -p %s/power 2> /dev/null' % cfg.general.resDir)
