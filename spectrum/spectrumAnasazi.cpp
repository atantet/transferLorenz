...
int readBinaryCrsMatrix(const char *filename, const Epetra_Map &rowMap,
...
int main(int argc, char * argv[])
{
  ...
  try {
    ...
    std::cout << "Reading transition matrix and transpose..."
	      << std::endl;
    Epetra_CrsMatrix *PT;
    if (readBinaryCrsMatrix(forwardTransitionFileName,
			    Map, PT, true)) {
      printer.print (Anasazi::Errors,
		     "readBinaryCrsMatrix::setProblem() \
reported an error.\n");
      throw -1;
    }
    std::cout << "=============================================\
